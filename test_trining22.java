...
//		3. 가로길이의 아이스크림틀 입력
		...
		for (int i = 0; i < n; i++) { // 0의 좌표 특정을 위한 for문
			for (int j = 0; j < m; j++) {
				if (icearr[i][j] == 0) {
					int n2 = i; // 확인용 세로칸 반복계산용변수
					int m2 = j; // 확인용 가로칸 반복계산용변수
					boolean es = false; // 동서남북이 각각 0이라면  false 1이라면 true
					boolean ws = false;
					boolean ss = false;
					boolean ns = false;

					while (es != true && ws != true && ss != true && ns != true) { //동서남부 모두 1이라면 종료

						try {
							if (icearr[n2][m2 - 1] == 0) { //0의 서쪽칸이 0인지 확인(사족:변수명 을 동쪽으로 지정함(문제는 없음)
								icearr[n2][m2 - 1] = 1;
								es = false;
								if (m2 > 0) {
									m2 = m2 - 1;
							...
							if (icearr[n2][m2 + 1] == 0) { //0의 동쪽칸이 0인지 확인(사족:변수명 을 서쪽으로 지정함(문제는 없음)
								icearr[n2][m2 + 1] = 1;
								ws = false;
							...
							if (icearr[n2 - 1][m2] == 0) { //0의 북쪽칸이 0인지 확인(사족:변수명 을 남쪽으로 지정함(문제는 없음)
								icearr[n2 - 1][m2] = 1;
								ss = false;
							...
							if (icearr[n2 + 1][m2] == 0) { //0의 남쪽칸이 0인지 확인(사족:변수명 을 북쪽으로 지정함(문제는 없음)
								icearr[n2 + 1][m2 - 1] = 1;
								ns = false;
						...
						} catch (Exception e) {// 지정범위를 벗어나면 중지하기 위함
							es = true;
							ws = true;
							ss = true;
							ns = true;
